document.addEventListener('DOMContentLoaded', async function () {
    ...
    const resultadosContainer = document.getElementById('resultados'); //Contenedor donde se mostrarán los resultados de los turnos
    const datosPacienteContainer = document.getElementById('datosPaciente'); //Contenedor con la información del paciente
    ...
    //console.log(window.matricula + ' - matrícula en front'); //Log para depurar
    ...
    searchInput.addEventListener('input', () => {
        ...
        if (query.length > 0) {
            ...
            resultadosContainer.innerHTML = ''; //Limpiar resultados si no hay búsqueda
        }
    });
    ...
    const mostrarTurnosHce = (turnosHistoria) => {
        ...
        turnosHistoria.forEach(turno => {
            ...
            tr.innerHTML = `
              ...
              <td class="turno-cell">${esMismoMedico ? (turno.diagnosticos.map((d, index) => `${index + 1}- ${d.resumen_diagnostico}`).join('<br>')) : 'Privado'}</td>
          ...
          `;
            ...
            if (esMismoMedico && !ultimoTurnoMarcado) {
                ...
                //console.log('marco el primero') //Log para depurar
    ...
    buscarButton.addEventListener('click', async () => {
        const dni = searchInput.value.match(/\d{8}$/); //Validar que el DNI sea correcto
        if (!dni) {
            Swal.fire({
                title: 'Error',
